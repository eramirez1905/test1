CREATE OR REPLACE TABLE rl.scorecard_rider_compliance_zones AS
WITH parameters AS (
  ...
  SELECT a.country_code
    , o.city_id
    , o.zone_id
    , FORMAT_DATE('%G-%V', CAST(DATETIME(a.created_at, a.timezone) AS DATE)) AS report_week_local
  ...
  LEFT JOIN dataset o ON a.country_code = o.country_code
  ...
  SELECT d.country_code
    , d.city_id
    , d.zone_id
    , FORMAT_DATE('%G-%V', d.report_date_local) AS report_week_local
  ...
  SELECT s.country_code
    , s.city_id
    , s.zone_id
    , FORMAT_DATE('%G-%V', DATE(DATETIME(s.shift_start_at, s.timezone))) AS report_week_local
...
LEFT JOIN shifts s ON d.country_code = s.country_code
  ...
  AND d.zone_id = s.zone_id
  AND d.report_week_local = s.report_week_local
LEFT JOIN logs l ON d.country_code = l.country_code
  AND d.city_id = l.city_id
  AND d.zone_id = l.zone_id
  AND d.report_week_local = l.report_week_local
;
