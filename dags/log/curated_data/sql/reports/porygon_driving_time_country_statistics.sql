CREATE OR REPLACE TABLE `{{ params.project_id }}.rl.porygon_driving_time_country_statistics` AS
WITH dataset AS (
...
SELECT *
  , (deliveries / (SUM(deliveries) OVER (PARTITION BY country_code, entity_display_name, porygon_vehicle_profile)) * 100) AS perc_deliveries
FROM aggregations
