...
default_args = {
    'owner': 'data-bi',
    'start_date': datetime(2020, 8, 1, 12, 0, 0),
    'retries': 2,
    'concurrency': 20,
    'max_active_runs': 1,
...
dag_id_prefix = 'curated-data-hourly'

doc_md = f"""
#### Create Curated Data on BigQuery - Hourly
"""
...
with DAG(
        ...
        schedule_interval=config.get('dags').get(dag_id_prefix, {}).get('schedule_interval', '15 */3 * * *'),
    ...
    etl = ProcessCuratedData(
        ...
        dataset_id='cl',
        config=config.get('curated_data_hourly'),
