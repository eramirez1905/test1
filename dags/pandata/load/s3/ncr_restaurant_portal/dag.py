"""
Loads NCR Restaurant Portal tables from S3 to GCS to BigQuery
"""
...
from load.s3.constants import S3_BUCKET_NCR, RPS_AP_DIR
from load.s3.utils.tasks import load_table_and_update

with DAG(
    "load_ncr_restaurant_portal_v1",
    ...
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        s3_bucket=S3_BUCKET_NCR,
        gcs_bucket=CONFIG.gcp.bucket_ncr_export,
        ...
        object_conditions={"includePrefixes": [f"{RPS_AP_DIR}/"]},
    )

    LOAD_DATASET = CONFIG.gcp.bigquery.get(NCR_RESTAURANT_PORTAL_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(NCR_RESTAURANT_PORTAL_LATEST_DATASET)
    for t in FINAL_DATASET.tables:
        load_table_and_update(
            ...
            key_prefix="00",
            bucket_name=CONFIG.gcp.bucket_ncr_export,
