"""
Load cybersource tables from S3 to GCS to BigQuery.
"""
...
from load.s3.constants import CYBERSOURCE_DIR, S3_BUCKET_PANDORA
...
with DAG(
    ...
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        object_conditions={"includePrefixes": [f"{CYBERSOURCE_DIR}/"]},
    )

    LOAD_DATASET = CONFIG.gcp.bigquery.get(CYBERSOURCE_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(CYBERSOURCE_LATEST_DATASET)
    for table in FINAL_DATASET.tables:
        load_table_and_update(
            dir_name=CYBERSOURCE_DIR,
            table_id=table.name,
