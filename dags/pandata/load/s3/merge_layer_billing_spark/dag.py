"""
Syncs S3 with GCS buckets. Then, loads tables from merge layer
billing and does minor transformations to clean invalid data with spark
...
from configs.constructors.table import RawDataProcBigQueryTable
...
from load.s3.merge_layer_billing_spark.constants import (
    DATAPROC_NAME,
...
with DAG(
    "load_merge_layer_billing_spark_v1",
    ...
    LOAD_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_BILLING_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_BILLING_LATEST_DATASET)
    TABLE_NAMES = [
        t.name for t in FINAL_DATASET.tables if isinstance(t, RawDataProcBigQueryTable)
    ]
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        object_conditions={
            "includePrefixes": [f"{ML_BA_AP_DIR}/{t}/" for t in TABLE_NAMES]
        },
    )
    ...
    for t in TABLE_NAMES:
        ...
        preprocess_and_load = DataProcPySparkOperator(
            ...
            arguments=[
                (
                    f"gs://{CONFIG.gcp.bucket_pandora_export}/{ML_BA_AP_DIR}/{t}/"
                    "data*.parquet"
                ),
                "PARQUET",
                load_table,
