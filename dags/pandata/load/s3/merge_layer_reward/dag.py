"""
Loads merge layer reward tables from S3 to GCS to BigQuery
"""
...
from load.s3.constants import S3_BUCKET_PANDORA, ML_RW_AP_DIR
from load.s3.utils.tasks import load_table_and_update

with DAG(
    "load_merge_layer_reward_v1",
    ...
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        object_conditions={"includePrefixes": [f"{ML_RW_AP_DIR}/"]},
    )

    LOAD_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_REWARD_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_REWARD_LATEST_DATASET)
    for t in FINAL_DATASET.tables:
        load_table_and_update(
            dir_name=ML_RW_AP_DIR,
            table_id=t.name,
