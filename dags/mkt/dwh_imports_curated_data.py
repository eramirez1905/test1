...
dag_id_prefix = 'dwh-imports-curated-data'
...
with DAG(
        ...
        schedule_interval=config.get('dags').get(dag_id_prefix, {}).get('schedule_interval', '0 2 * * *'),
    ...
    etl = ProcessCuratedData(
        ...
        config=config.get('dwh_imports_curated_data'),
