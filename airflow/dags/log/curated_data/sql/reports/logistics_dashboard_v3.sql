CREATE OR REPLACE TABLE `{{ params.project_id }}.rl.logistics_dashboard_v3`
PARTITION BY report_date
CLUSTER BY country_code, entity_display_name, city_name AS
WITH parameters AS (
  ...
  SELECT 'hurrier_orders' AS _source
    ...
    , IF(d.vertical_type = 'restaurants' AND d.delivery_status = 'completed',  d.order_id, NULL) AS food_order
    , IF(d.vertical_type <> 'restaurants' AND d.delivery_status = 'completed', d.order_id, NULL) AS non_food_order
    , IF(d.vertical_type IS NULL AND d.delivery_status = 'completed', d.order_id, NULL) AS non_tagged_order
  ...
  SELECT 'rooster' AS _source
    ...
    , NULL AS food_order
    , NULL AS non_food_order
    , NULL AS non_tagged_order
  ...
  SELECT region
    ...
    , COUNT(food_order) AS food_orders
    , COUNT(non_food_order) AS non_food_orders
    , COUNT(non_tagged_order) AS non_tagged_orders
...
SELECT region
  ...
  , NULL AS food_orders
  , NULL AS non_food_orders
  , NULL AS non_tagged_orders
