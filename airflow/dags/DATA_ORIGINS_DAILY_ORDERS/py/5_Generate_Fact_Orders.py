...
def generateFactOrdersDetailsOptions(app_args):
    ...
    s3_orders_cleaned = app_args.s3_source_path
    s3_fact_orders = app_args.s3_destination_path
    ...
    s3_refined_order_state = 's3://peyabi.datalake.live/bi-core-tech/refined/dim_order_state/'
    s3_refined_application = 's3://peyabi.datalake.live/bi-core-tech/refined/dim_application/'
    s3_refined_business_type = 's3://peyabi.datalake.live/bi-core-tech/refined/dim_business_type/'
    ...
    conf = SparkConf().setAppName("ETL__fact_orders")
    ...
    for day in partitions:
        s3_orders_data_paths.append(
            '{0}/yyyymmdd={1}'.format(s3_orders_cleaned, day))
        print('<<-------- Orders raw data source: {0}/yyyymmdd={1}'.format(s3_orders_cleaned, day))

    # Lectura de toda la data de ordenes
    orders = spark.read.parquet(*s3_orders_data_paths)
 ...
 ## Procesamiento fact_orders ###########################################################################################

 ### STG DATA

    #### STG_COMMISSION_INCLUDES_DELIVERY_COST
    ##### Calculo de la comisión para Colombia, cambio realizado el 31/10/2019. Jira BCT-314

    stg_commission_includes_delivery_cost = orders.withColumn('commission_includes_delivery_cost', when(
    (col('yyyymmdd') >= 20190926) & (col('restaurant.country.id') == '6'), False)
                                                              .otherwise(col('commissionIncludesDeliveryCost'))).select(
        col('id').alias('order_id'), col('commission_includes_delivery_cost'))

    stg_commission_includes_delivery_cost.createOrReplaceTempView("stg_commission_includes_delivery_cost")

    ### stg_discountpaidbycompany_totaldiscount
    ##### Calculo de total amount de los descuentos asociados a la Orden

    stg_discountpaidbycompany_totaldiscount = orders.select(col('id').alias('order_id'),
                                              explode_outer(col('discounts')).alias('_discount_struct') # Genera registro por elemento del array
                                             ).filter(col('_discount_struct.id').isNotNull())

    stg_discountpaidbycompany_totaldiscount = stg_discountpaidbycompany_totaldiscount.groupBy(col('order_id')).agg(sum(col('_discount_struct.amount')).alias('totalDiscount'),
                                                                    sum(when(col('_discount_struct.paidBy')=='COMPANY', col('_discount_struct.amount')).otherwise(0)).alias('discountPaidByCompany'))


    stg_discountpaidbycompany_totaldiscount.createOrReplaceTempView("stg_discountpaidbycompany_totaldiscount")


    #### STG_COMMISSIONABLE_AMOUNT - JOKER #############################################################################
    ###### Calculo del commissionable_amount por joker. para calculo de la comision

    stg_commissionable_amount = orders.select(col('id').alias('order_id'),
                                              col('totalAmount'),
                                              explode_outer(col('discounts')).alias('_discount_struct')
                                              # Genera registro por elemento del array
                                              ).filter(col('_discount_struct.id').isNotNull()).filter(
        col('_discount_struct.type') == 'JOKER')

    stg_commissionable_amount = stg_commissionable_amount.select(col('order_id'),
                                                                 when(col('_discount_struct.amount') == 0,
                                                                      col('totalAmount') - col(
                                                                          '_discount_struct.originalValue'))
                                                                 .otherwise((col('_discount_struct.amount') + col(
                                                                     'totalAmount')) / 2)
                                                                 .alias('commissionable_amount')
                                                                 )

    stg_commissionable_amount.createOrReplaceTempView("stg_commissionable_amount")

    sqlcontext.cacheTable("stg_commissionable_amount")
    ...
    pre_fact_orders_joins = sqlcontext.sql("""
                select 
                    ...
                    IFNULL(stg_discountpaidbycompany_totaldiscount.totalDiscount,0) AS discount_amount,
                    
                    (
                        IFNULL(pre_fact_orders.total_amount,0) 
                        + IFNULL(shipping_amount,0)
                    ) as total_amount_with_dc,

                    CASE 
                        WHEN pre_fact_orders.registered_date >= '2019-01-01' 
                        THEN
                            CASE WHEN (pre_fact_orders.registered_date >= '2019-04-01' AND pre_fact_orders.country_id = 6) THEN
                            (		
                                (
                                    CASE 
                                        WHEN (stg_commission_includes_delivery_cost.commission_includes_delivery_cost = 0) THEN (coalesce(stg_commissionable_amount.commissionable_amount,pre_fact_orders.total_amount) + IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0) - pre_fact_orders.shipping_amount_no_discount + pre_fact_orders.shipping_amount)*(pre_fact_orders.commission/100) 
                                        ELSE (coalesce(stg_commissionable_amount.commissionable_amount,pre_fact_orders.total_amount)  + IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0) + pre_fact_orders.shipping_amount)*(pre_fact_orders.commission/100) END
                                )
                                + 
                                (
                                    CASE 
                                        WHEN pre_fact_orders.with_logistics THEN ((coalesce(stg_commissionable_amount.commissionable_amount,pre_fact_orders.total_amount)  + IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0) - pre_fact_orders.shipping_amount_no_discount + pre_fact_orders.shipping_amount)*(pre_fact_orders.logistics_commission)/100)
                                        ELSE 0 END
                                )
                            )
                            ELSE
                            (
                                (
                                    (case when stg_commission_includes_delivery_cost.commission_includes_delivery_cost <> 1 OR IFNULL(pre_fact_orders.with_logistics,False) = 1 then - pre_fact_orders.shipping_amount_no_discount else 0 end )
                                    + pre_fact_orders.shipping_amount + coalesce(stg_commissionable_amount.commissionable_amount,pre_fact_orders.total_amount)  + IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0)
                                )  * (pre_fact_orders.commission + IF(pre_fact_orders.with_logistics,IFNULL(pre_fact_orders.logistics_commission,0),0))/100
                            ) end 	
                        ELSE 
                            (
                                (case when stg_commission_includes_delivery_cost.commission_includes_delivery_cost=1 AND IFNULL(pre_fact_orders.with_logistics,False) <> 1 then pre_fact_orders.shipping_amount else 0 end )  
                                + coalesce(stg_commissionable_amount.commissionable_amount,pre_fact_orders.total_amount)  + IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0)
                            )  * (pre_fact_orders.commission + IF(pre_fact_orders.with_logistics,IFNULL(pre_fact_orders.logistics_commission,0),0))/100 END 
                    AS commission_amount,

                    stg_commission_includes_delivery_cost.commission_includes_delivery_cost,
                    IFNULL(stg_discountpaidbycompany_totaldiscount.discountPaidByCompany,0) AS discount_paid_by_company,

                    CAST(stg_visualization_date.visualization_date AS Timestamp) as visualization_date, 
                    stg_visualization_date.visualization_date_id,
                    
                    CAST(stg_transmitting_date.transmitting_date AS Timestamp) as transmitting_date,
                    stg_transmitting_date.transmitting_date_id,

                    CAST(stg_reception_offset.reception_date AS Timestamp) as reception_date,
                    stg_reception_offset.reception_date_id,
                ...
                FROM pre_fact_orders
                    ...
                    left JOIN stg_commission_includes_delivery_cost stg_commission_includes_delivery_cost 
                        ON stg_commission_includes_delivery_cost.order_id = pre_fact_orders.order_id 

                    LEFT JOIN stg_discountpaidbycompany_totaldiscount stg_discountpaidbycompany_totaldiscount 
                        ON pre_fact_orders.order_id = stg_discountpaidbycompany_totaldiscount.order_id

                    LEFT JOIN stg_commissionable_amount AS stg_commissionable_amount 
                        ON stg_commissionable_amount.order_id = pre_fact_orders.order_id   

                    LEFT JOIN stg_visualization_date as stg_visualization_date 
                        ON stg_visualization_date.order_id = pre_fact_orders.order_id

                    LEFT JOIN stg_reception_offset as stg_reception_offset 
                        ON stg_reception_offset.order_id = pre_fact_orders.order_id
                        
                    LEFT JOIN stg_transmitting_date as stg_transmitting_date
                        ON stg_transmitting_date.order_id = pre_fact_orders.order_id
    ...
    fact_orders = pre_fact_orders_joins.distinct().cache()

    fact_orders = fact_orders.orderBy('order_id', ascending=True)

    #### Columna de Partición
    fact_orders = fact_orders.withColumn('yyyymmdd', col('registered_date_id'))

    # Particionado por yyyymmdd de registered_date
    fact_orders = fact_orders.repartition(fact_orders.yyyymmdd)

    print('** fact_orders partición OK, procede escribir parquets en S3')

    # Generación de parquets por partición
    for day in partitions:
        to_parquet = '{0}/yyyymmdd={1}'.format(s3_fact_orders, day)

        partition = fact_orders.filter(col('yyyymmdd') == day)

        print("*** Procesando *** \n [bucket/particion: {0} | ejecución: {1}]".format(to_parquet, datetime.strftime(
            datetime.now() - timedelta(hours=3), "%Y-%m-%d %H:%M:%S")))
