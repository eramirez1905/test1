"""
Loads merge layer refer a friend tables from S3 to GCS to BigQuery
"""
...
from load.s3.constants import S3_BUCKET_PANDORA, ML_RAF_AP_DIR
from load.s3.utils.tasks import load_table_and_update

with DAG(
    "load_merge_layer_refer_a_friend_v1",
    ...
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        object_conditions={"includePrefixes": [f"{ML_RAF_AP_DIR}/"]},
    )

    LOAD_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_REFER_A_FRIEND_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(MERGE_LAYER_REFER_A_FRIEND_LATEST_DATASET)
    for t in FINAL_DATASET.tables:
        load_table_and_update(
            dir_name=ML_RAF_AP_DIR,
            table_id=t.name,
