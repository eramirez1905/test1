"""
Loads intelligence layer tables from S3 to GCS to BigQuery
"""
...
from load.s3.constants import S3_BUCKET_PANDORA, IL_REGION_AP_DIR
from load.s3.utils.tasks import load_table_and_update

with DAG(
    "load_intelligence_layer_v1",
    ...
    sync_s3_to_gcs = S3ToGoogleCloudStorageTransferOperator(
        ...
        object_conditions={"includePrefixes": [f"{IL_REGION_AP_DIR}/"]},
    )

    LOAD_DATASET = CONFIG.gcp.bigquery.get(INTELLIGENCE_LAYER_DATASET)
    FINAL_DATASET = CONFIG.gcp.bigquery.get(INTELLIGENCE_LAYER_LATEST_DATASET)
    for t in FINAL_DATASET.tables:
        load_table_and_update(
            dir_name=IL_REGION_AP_DIR,
            table_id=t.name,
