...
dag_name = "DATA_ORIGINS_DH_S3_TO_BQ_{0}_DAG"
...
cluster_init = {  # COMPLETAR CONFIGS
    'Name': '@dagName_{{ ds_nodash }}',
    'Tags': [
        {
            "Key": "team",
            "Value": our_team
    ...
    'LogUri': 's3://@s3_utils_bucket/@home_dir/@dag_folder/logs/@model/@date_id/',
     'BootstrapActions': [
        {
            'Name': 'sync_emr_cluster',
            'ScriptBootstrapAction': {
                'Path': 's3://{0}/{1}/{2}/bootstrap_actions/cluster_bootstrap.sh'.format(s3_utils_bucket, home_dir, dag_folder)
            }
        }
    ]
}
    ...
    new_templeate = {
        ...
        'HadoopJarStep': {
            ...
            'Args': [
                ...
                "--driver-cores", "4",
                "--executor-cores", "4",
                "s3://@s3_utils_bucket/@home_dir/@dag_folder/spark_scripts/@script",
                "--source_path", 's3://@source_dir',
                "--live_path", 's3://@target_dir',
                "--partition", '@date_id',
                "--model", '@model',
...
dag_path = "{0}/airflow/dags/OPSLOG_PANDORA_S3_TO_DL_DWH".format(git_repo_path)
...
def createNewDummyOperator(pDag, pName):
    return DummyOperator(task_id='dummy_{0}'.format(pName), dag=pDag)
...
def createNewStepChecker(pDag, stepNumber, stepObj):
    return EmrStepSensor(
        task_id='watch_step_' + stepObj['Name'],
        job_flow_id="{{ task_instance.xcom_pull('create_job_flow', key='return_value') }}",
        step_id="{{ task_instance.xcom_pull(task_ids='add_steps', key='return_value')[@index] }}".replace('@index', str(
            stepNumber)),
        aws_conn_id='aws_emr',
        mode='reschedule',
        poke_interval=5*60,
...
def createSubDag(dagName, pSubDagName, dConfig, sparkSteps, clusterInit):

    dag = DAG(dag_id=dagName + '.' + pSubDagName,
              start_date=datetime(dConfig['start_year'], dConfig['start_month'], dConfig['start_day']),
              default_args=default_args_subdag,
              schedule_interval=None,
              max_active_runs=1)

    clusterInit['Name'] = '@dagName_{{ ds_nodash }}'.replace('@dagName', dagName)
    cluster_creator = EmrCreateJobFlowOperator(
        task_id='create_job_flow',
        job_flow_overrides=clusterInit,
        aws_conn_id='aws_emr',
        #emr_conn_id='emr_transient_medium',
        emr_conn_id='emr_transient_small',
        dag=dag
    )
    ...
    step_checker_list = []
    for num, step in enumerate(sparkSteps, start=0):
        step_to_watch = createNewStepChecker(dag, num, step)
        step_checker_list.append(step_to_watch)
        # airflow.utils.helpers.chain(*[step_adder, step_to_watch])
    ...
    # call_clear_subdag = TriggerDagRunOperator(
    #     task_id="cal_retrigger_dag",
    #     trigger_dag_id="OPSLOG_RETRIGGER_SUBDAG_PANDORA",
    #     execution_date='{{ execution_date }}',
    #     trigger_rule = TriggerRule.ONE_FAILED,
    #     dag=dag
    # )

    remove_cluster_success = EmrTerminateJobFlowOperator(
        task_id='remove_cluster_success',
        trigger_rule=TriggerRule.ALL_SUCCESS,
        job_flow_id="{{ task_instance.xcom_pull(task_ids='create_job_flow', key='return_value') }}",
        aws_conn_id='aws_emr',
        retries=2,
    ...
    # remove_cluster_fail = EmrTerminateJobFlowOperator(
    #     task_id='remove_cluster_fail',
    #     trigger_rule=TriggerRule.ONE_FAILED,
    #     job_flow_id="{{ task_instance.xcom_pull(task_ids='create_job_flow', key='return_value') }}",
    #     aws_conn_id='aws_emr',
    #     dag=dag
    ...
    # step_adder >> step_checker_list >> remove_cluster_success
    cluster_creator >> step_adder >> step_checker_list >> remove_cluster_success
    # step_checker_list >> call_clear_subdag
    return dag

def createSubDagOperator(pParentDag, pDagName, pDConfig, pClusterInit):
    tid_subdag = 'subdag_' + pDConfig['model']
    spark_steps = []
    separator = ','
    # if pDConfig['has_main_table']:
    # if pDConfig['partitioning']:
    #     partition_param = str(pDConfig['partitioning'])
    # else:
    #     partition_param = "None"
    # if True:
    # CONVERSIONS
    if pDConfig['types_conversion']:
        aux = []
        separator = ","
        for key, value in pDConfig['types_conversion'].items():
    ...
    # PARTITION COLUMN
    if pDConfig['filter_column']:
        filter_col = pDConfig['filter_column']
    else:
        filter_col = 'False'

    # COUNTRY ID COLUMN
    if pDConfig['country_col']:
        country_col = pDConfig['country_col']
    else:
        country_col = 'False'

    tempTemplate = getTemplate()
    tempTemplate['Name'] = tempTemplate['Name'].replace('@model', pDConfig['model'])
    tempTemplate['HadoopJarStep']['Args'] = [arg.replace('@script', pDConfig['spark_script'])
                                                 .replace('@s3_utils_bucket', s3_utils_bucket)
                                                 .replace('@home_dir', home_dir).replace('@dag_folder', dag_folder)
                                                 .replace('@source_dir',
                                                          pDConfig['source_bucket'] + '/' + pDConfig[
                                                              'source_dir'] + '/' +
                                                          pDConfig['source_folder'])
                                                 .replace('@target_dir',
                                                          s3_live_bucket + '/' + home_dir + '/' + pDConfig['target_dir'] + '/' +
                                                          pDConfig['target_folder'])
                                                 .replace('@date_id', pDConfig['date_id'])
                                                 .replace('@model', pDConfig['model'])
                                                 .replace('@conversions', conversion_string)
                                                 .replace('@partition_col', filter_col)
                                                 .replace('@country_col', country_col)
                                                 .replace('@overwrite_mode', pDConfig.get('overwrite_mode') or 'static')
                                             for arg in tempTemplate['HadoopJarStep']['Args']]

    spark_steps.append(tempTemplate)

    pClusterInit['Name'] = pClusterInit['Name'].replace('@model', pDConfig['model'])
    pClusterInit['LogUri'] = pClusterInit['LogUri'].replace('@s3_utils_bucket', s3_utils_bucket)\
                                                    .replace('@home_dir', home_dir)\
                                                    .replace('@dag_folder', dag_folder)\
                                                    .replace('@model', pDConfig['model'])\
                                                    .replace('@date_id', '{{ ds_nodash}}')

    new_subdag = createSubDag(pDagName, tid_subdag, pDConfig, spark_steps, pClusterInit)
...
def createDAGFull(dConfig, dagName):
    ...
    # *** FOR GENERAL USE ***
    gcs_target = gcs_bucket + '/' + dConfig['target_dir']
    ...
    add_tasks = createNewDummyOperator(new_dag, 'add_tasks')
...
with open('{0}/dag_bag_config.json'.format(config_path)) as json_file:
