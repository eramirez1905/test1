select restaurant_id,
		case 
			when n_day_weekday + n_day_weekend > 0 then 1 
			else 0 
		end 								as "day",
		n_day_weekday,
		n_day_weekend,
		case 
			when n_night_weekday + n_night_weekend > 0 then 1 
			else 0 
		end 								as "night",
		n_night_weekday,
		n_night_weekend
...
SELECT
		restaurant_id,
		count(distinct case 
							when extract(hour from open_from_date) <= 13 
									and extract(hour FROM open_to_date) +
				(case 
						when extract(day from open_to_date) > 1 then (extract(day from open_to_date)-1)*24 
						ELSE 1 
				  end) >= 13 and
...
HAVING min(schedule_day_id) is not null 
		AND min(open_from_date) is not null 
		AND dateadd(day, sum(dif), dateadd(day, -extract(day from max(open_to_date)) + 1,
