DROP TABLE `peya-data-origins-stg.origin_data_refined.country_details_origin`;
CREATE OR REPLACE TABLE
  `peya-data-origins-stg.origin_data_refined.country_details_origin`
  PARTITION BY RANGE_BUCKET(country_id, GENERATE_ARRAY(0, 20, 1))
  CLUSTER BY country_id, country_name, active
	AS 
    SELECT 
            cd.country_id,
            cd.country_name,
