DELETE FROM `peya-data-origins-stg.origin_data_refined.user_origin`
            WHERE user_id IN (SELECT id 
            FROM `peya-data-origins-stg.origins_data_stg.sqs_user`);

INSERT INTO `peya-data-origins-stg.origin_data_refined.user_origin`
    SELECT 
            u.id AS user_id
    ...
    DELETE FROM `peya-data-origins-stg.origin_data_refined.user_origin`
        WHERE user_id in (2137876,2376652,5034603,6181138,7343509,7343547,13682130,13839402,13870971,14003043,14003126,14003176)
        OR user_type = 15;