...
class VariableImportancesInfraOperator(BaseOperator):
    template_ext = ['.sql']
    ...
    def __init__(self, bigquery_conn_id='bigquery_default', *args, **kwargs):
        ...
        self.target_table = 'rl.variable_importances_infra'
    ...
    def execute(self, context):
        sql = self.render_template('side_tables/side_infra_weights.sql', context)
    ...
    def remove_margins(self, df):
        ...
        df = df[df.pickup_distance < df.pickup_distance.quantile(0.99)]
        df = df[df.dropoff_distance < df.dropoff_distance.quantile(0.99)]
        df = df[df.dropoff_accuracy < df.dropoff_accuracy.quantile(0.99)]
        df = df[df.pickup_accuracy < df.pickup_accuracy.quantile(0.99)]
        df = df[df.utr < 5]
        return df
