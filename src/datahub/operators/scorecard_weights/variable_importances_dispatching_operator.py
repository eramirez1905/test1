...
class VariableImportancesDispatchingOperator(BaseOperator):
    template_ext = ['.sql']
    ...
    def __init__(self, bigquery_conn_id='bigquery_default', *args, **kwargs):
        ...
        self.target_table = 'rl.variable_importances_dispatching'
    ...
    def execute(self, context):
        sql = self.render_template('side_tables/side_dispatching_weights.sql', context)
    ...
    def remove_margins(self, df):
        ...
        df = df[df.avg_issue_solving_time < df.avg_issue_solving_time.quantile(0.99)]
        df = df[df.perc_manually_touched_orders < df.perc_manually_touched_orders.quantile(0.99)]
        df = df[df.utr <= 5]
        return df
