...
class VariableImportancesVendorComplianceOperator(BaseOperator):
    template_ext = ['.sql']
    ...
    def __init__(self, bigquery_conn_id='bigquery_default', *args, **kwargs):
        ...
        self.target_table = 'rl.variable_importances_vendor'
    ...
    def execute(self, context):
        sql = self.render_template('side_tables/side_vendor_compliance_weights.sql', context)
    ...
    def remove_margins(self, df):
        ...
        df = df[df.cancellation_rate < df.cancellation_rate.quantile(0.99)]
        df = df[df.estimated_prep_duration < df.estimated_prep_duration.quantile(0.99)]
        df = df[df.reliability_rate < df.reliability_rate.quantile(0.99)]
        return df
