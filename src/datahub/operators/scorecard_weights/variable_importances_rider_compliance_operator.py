...
class VariableImportancesRiderComplianceOperator(BaseOperator):
    template_ext = ['.sql']
    ...
    def __init__(self, bigquery_conn_id='bigquery_default', *args, **kwargs):
        ...
        self.target_table = 'rl.variable_importances_rider'
    ...
    def execute(self, context):
        sql = self.render_template('side_tables/side_rider_compliance_weights.sql', context)
    ...
    def remove_margins(self, df):
        ...
        df = df[df.utr < df.utr.quantile(0.975)]
        df = df[df.perc_reactions_over_2 < df.perc_reactions_over_2.quantile(0.99)]
        df = df[df.perc_vendor_time_over_7 < df.perc_vendor_time_over_7.quantile(0.99)]
        df = df[df.perc_customer_time_over_5 < df.perc_customer_time_over_5.quantile(0.99)]
        df = df[df.acceptance_issues < df.acceptance_issues.quantile(0.99)]
        df = df[df.perc_no_show_shifts < df.perc_no_show_shifts.quantile(0.99)]
        df = df[df.perc_full_work_shifts < df.perc_full_work_shifts.quantile(0.99)]
        return df
